...
public class AuctionEventListener {
    ...
    private void processEvent(AuctionEvent event, Message message, Channel channel, Consumer<Long> auctionAuction, String eventType) {
        try {
            ...
            if (retryCount >= 3) {
                log.error("[RabbitMQ] {} 처리 중 시스템 예외 3회 초과 - auctionId={}", eventType, event.getAuctionId(), e);

                ackMessage(channel, message);
                return;
            }
            log.error("[RabbitMQ] {} 처리 중 시스템 예외 발생 - auctionId={}, retryCount={}", eventType, event.getAuctionId(), retryCount, e);

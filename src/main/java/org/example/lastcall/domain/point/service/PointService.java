...
import org.example.lastcall.domain.point.enums.PointErrorCode;
...
public class PointService implements PointServiceApi {
    ...
    public void updateDepositPoint(Long auctionId, Long bidId, Long bidAmount, Long userId) {
        ...
        Optional<Bid> existingBid = bidQueryServiceApi.getLastBidExceptBidId(auctionId, userId, bidId);
        ...
        if (existingBid.isPresent()) {
            ...
            if (bidAmount > previousBidAmount) {
                ...
                PointLog log = PointLog.create(
                        ...
                        difference,
                        auctionServiceApi.getUserId(auctionId)
                );
    ...
    public void depositToSettlement(Long userId, Long auctionId, Long amount) {
        ...
        // 경매 및 최고 입찰 조회
        Auction auction = auctionServiceApi.getUserId(auctionId);
        Bid highestBid = bidQueryServiceApi.getTopByAuctionOrderByBidAmountDesc(auction).orElseThrow(

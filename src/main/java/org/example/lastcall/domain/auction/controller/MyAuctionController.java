...
public class MyAuctionController {
    ...
    // 내가 판매한 경매 목록 조회 //
    @GetMapping("/selling")
    public ResponseEntity<ApiResponse<PageResponse<MyAuctionReadAllResponse>>> getMySellingAuctions(
            //@AuthenticationPrincipal AuthUser authUser,
            // AuthUser 적용되면 email -> user.getId()
            Pageable pageable) {
        Long testUserId = 5L; // DB에 실제 존재하는 user_id (임시) -> authUser 적용 후 삭제하기
        PageResponse<MyAuctionReadAllResponse> pageResponse = myAuctionService.getMySellingAuctions(testUserId, pageable);
